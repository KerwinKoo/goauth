...
import (
	"encoding/json"
	"fmt"
	"github.com/KerwinKoo/goauth/client"
...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "answerPing",
		Short: `answer wifidog ping GET-Request`,
	}
	tmp1 := new(AnswerPingPingCommand)
	sub = &cobra.Command{
		Use:   `ping ["/wifidog/ping/ARGS"]`,
		Short: ``,
...
func (cmd *AnswerPingPingCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/wifidog/ping/%v", url.QueryEscape(cmd.Args))
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.AnswerPingPing(ctx, path)
	if err != nil {
...
func (cmd *AnswerPingPingCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var args string
	cc.Flags().StringVar(&cmd.Args, "args", args, ``)
}
